...
} from '@/features/catalog/supplierAccounts'
import { FAMILIES, FAMILY_LABELS, FAMILY_COLORS } from '@/features/catalog/types'
import { parseFabdis } from '@/features/catalog/mappers/fabdis'
import { importEntries, countEntriesBySupplier, getLatestSnapshot, subscribeCatalog } from '@/features/catalog/store'
import { invoiceToDrafts } from '@/features/catalog/ocr/invoiceToDrafts'
...
function InvoiceTab({ supplierId, snapshot, onDone }: { supplierId: string; snapshot: CatalogSnapshot | null; onDone: () => void }) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const { invoice, fileName } = await res.json() as { invoice: import('@/features/catalog/ocr/schema').ExtractedInvoice; fileName: string }
